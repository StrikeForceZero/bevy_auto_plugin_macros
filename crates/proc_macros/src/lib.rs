...
impl AutoPluginAttributes {
    ...
    fn init_name(&self) -> Ident {
        self.init_name.as_ref().cloned().unwrap_or(Ident::new("init", Span::call_site()))
    }
}
...
fn auto_plugin_inner(mut module: ItemMod, init_name: &Ident) -> Result<MacroStream> {
    ...
    if let Some((_, items)) = &module.content {
        ...
        fn map_to_string(iter: impl IntoIterator<Item = ItemWithAttributeMatch>) -> impl Iterator<Item = String> {
            iter.into_iter().map(ItemWithAttributeMatch::into_path_string)
        }
        ...
        inject_module(&mut module, move || {
            let auto_register_types = generate_register_types(&app_param_ident, auto_register_types)?;
            let auto_add_events = generate_add_events(&app_param_ident, auto_add_events)?;
            let auto_init_resources = generate_init_resources(&app_param_ident, auto_init_resources)?;
            parse2::<Item>(quote! {
